...
const InputArea: React.FC = () => {
  ...
  return (
    <div className="absolute bottom-0 w-screen z-30 px-3 flex flex-col gap-2 pb-4">
      {/* Input Sheet Area */}
      {!isTyping && !selectedFile && mediaFrom && (
        <div className="bg-white rounded-4xl p-6">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            {/* <label className="bg-[#f0f0f0] p-4 rounded-lg flex justify-center items-center flex-col gap-1 cursor-not-allowed opacity-50">
      ...
      <div className="flex items-center gap-2">
        <div className="bg-white grow flex px-5 border border-slate-200 rounded-full gap-5">
          <input
            type="text"
        ...
        {hasContent ? (
          <button
            onClick={sendMessage}
            className="py-2 bg-slate-800 border-slate-200 border text-white h-14 w-14 rounded-full grid place-content-center disabled:opacity-50"
          >
          ...
          <button
            ...
            className="py-2 bg-white text-black border-slate-200 border h-14 w-14 rounded-full grid place-content-center"
          >
            <HugeiconsIcon icon={GoogleDocFreeIcons} />
          </button>
        )}
